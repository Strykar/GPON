...
| `IncompatiblePeer: Incompatible ssh peer (no acceptable kex algorithm)` | paramiko 5.x installed. paramiko 5 deleted the legacy kex and host-key algorithms the SFP's 2008-era dropbear requires; install `paramiko>=3.0,<5` (the pinned `requirements.txt` and the published container image do). See [QUIRKS](QUIRKS.md#legacy-ssh-crypto). |
